...
import { OpenDao } from "@0xlol/sdk";

import { config } from "@/config/index";
import { EventData } from "@/events-sync/data";

export const erc721OrderCancelled: EventData = {
  kind: "opendao-erc721-order-cancelled",
  addresses: { [OpenDao.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
...
export const erc1155OrderCancelled: EventData = {
  kind: "opendao-erc1155-order-cancelled",
  addresses: { [OpenDao.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
...
export const erc721OrderFilled: EventData = {
  kind: "opendao-erc721-order-filled",
  addresses: { [OpenDao.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
...
export const erc1155OrderFilled: EventData = {
  kind: "opendao-erc1155-order-filled",
  addresses: { [OpenDao.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
